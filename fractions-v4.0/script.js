...
window.addEventListener("load", () => {
  ...
  function startGame() {
    ...
    fractions.forEach(fr => {
      ...
      fractionContainer.appendChild(card);
    });
  ...
  function initDragAndDrop() {
    ...
    fractionContainer.addEventListener("touchstart", (e) => {
      const touch = e.touches[0];
      const target = document.elementFromPoint(touch.clientX, touch.clientY).closest(".fraction-card");
      if (target) {
        draggedCard = target;
        draggedCard.classList.add("dragging");
        touchData.startX = touch.clientX;
        touchData.startY = touch.clientY;
        touchData.originalIndex = Array.from(fractionContainer.children).indexOf(draggedCard);
        // Create a clone for visual feedback
        touchData.clone = draggedCard.cloneNode(true);
        touchData.clone.classList.add("clone");
        touchData.clone.style.position = "absolute";
        touchData.clone.style.pointerEvents = "none";
        touchData.clone.style.opacity = "0.7";
        touchData.clone.style.width = `${draggedCard.offsetWidth}px`;
        touchData.clone.style.height = `${draggedCard.offsetHeight}px`;
        touchData.clone.style.left = `${touch.clientX - draggedCard.offsetWidth / 2}px`;
        touchData.clone.style.top = `${touch.clientY - draggedCard.offsetHeight / 2}px`;
        document.body.appendChild(touchData.clone);
      }
    }, { passive: false });

    fractionContainer.addEventListener("touchmove", (e) => {
      if (!draggedCard || !touchData.clone) return;
    ...
    fractionContainer.addEventListener("touchend", () => {
      if (draggedCard && touchData.clone) {
        draggedCard.classList.remove("dragging");

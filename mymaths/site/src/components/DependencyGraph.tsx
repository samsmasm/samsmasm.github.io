...
export default function DependencyGraph({ modules, phases }: Props) {
  ...
  useEffect(() => {
    ...
    // Read CSS variables for font sizes so D3 respects the type scale
    const cs = getComputedStyle(document.documentElement)
    const textXs  = cs.getPropertyValue('--text-xs').trim()  || '14px'
    const text2xs = cs.getPropertyValue('--text-2xs').trim() || '12px'
    ...
    defs.append('marker')
      ...
      .append('path').attr('d', 'M0,-4L8,0L0,4').attr('fill', '#cdc8be')

    defs.append('marker')
      .attr('id', 'arrow-critical')
      .attr('viewBox', '0 -4 8 8').attr('refX', 24).attr('refY', 0)
      .attr('markerWidth', 5).attr('markerHeight', 5).attr('orient', 'auto')
      .append('path').attr('d', 'M0,-4L8,0L0,4').attr('fill', '#d97706')

    const nodes: GraphNode[] = modules.map(m => ({
      id: m.id, label: m.short_name, phase: m.phase,
      critical: m.critical_path, duration: m.duration_weeks,
      name: m.name, track: m.specialisation_track,
    }))

    const links: GraphLink[] = []
    modules.forEach(mod => {
      mod.prerequisites.forEach(prereqId => {
        const s = nodes.find(n => n.id === prereqId)
        const t = nodes.find(n => n.id === mod.id)
        if (s && t) links.push({ source: prereqId, target: mod.id, critical: s.critical && t.critical })
      })
    })
    ...
    const linkSel = g.append('g')
      .selectAll<SVGLineElement, GraphLink>('line').data(links).join('line')
    ...
    nodeSel.append('circle')
      .attr('r', d => Math.max(24, 20 + d.duration / 3))
      .attr('fill', d => phaseColorMap[d.phase] + '18')
      .attr('stroke', d => phaseColorMap[d.phase])
      .attr('stroke-width', d => d.critical ? 2.5 : 1.5)

    // Use CSS variable values read above so text scales with --text-* vars
    nodeSel.append('text')
      .attr('text-anchor', 'middle').attr('dy', '0.3em')
      .style('font-size', textXs).attr('font-weight', '600')
      .attr('fill', '#1a1714').attr('pointer-events', 'none')
      .text(d => d.label)

    nodeSel.append('text')
      .attr('text-anchor', 'middle').attr('dy', '-1.8em')
      .style('font-size', text2xs).attr('fill', '#a09890')
    ...
    nodeSel
      .on('mouseover', (event, d) => {
        ...
        tooltip!.innerHTML = `
          <div style="font-family:monospace;font-size:var(--text-2xs);color:${color};margin-bottom:4px;letter-spacing:0.06em;text-transform:uppercase">${d.id} · ${phase.label}</div>
          <div style="font-weight:600;margin-bottom:4px">${d.name}</div>
          <div style="color:#a09890;font-size:var(--text-2xs)">${d.duration} weeks${d.critical ? ' · ⚡ Critical' : ''}${d.track ? ` · Track ${d.track}` : ''}</div>
        `
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-2)' }}>Drag · scroll to zoom · click to open</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginLeft: 'auto', flexWrap: 'wrap' }}>
          {phases.map(p => (
            <span key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 'var(--text-2xs)', color: 'var(--text-3)' }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: p.color, display: 'inline-block' }} />
              {p.label}
            </span>
          ))}
          <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 'var(--text-2xs)', color: 'var(--text-3)' }}>

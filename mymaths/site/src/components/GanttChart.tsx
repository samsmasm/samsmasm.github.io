    ...
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 'auto', flexWrap: 'wrap' }}>
          {phases.map(p => (
            <span key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 'var(--text-2xs)', color: 'var(--text-3)' }}>
      ...
      <div style={{ overflowX: 'auto' }}>
        <div style={{ minWidth: 700, padding: '14px 20px 18px' }}>
          ...
          <div style={{ position: 'relative', height: 22, marginBottom: 8 }}>
            {ticks.map(w => (
              <div
                key={w}
                style={{
                  position: 'absolute', left: `${toPercent(w)}%`, top: 0,
                  color: 'var(--text-3)', fontFamily: 'var(--font-mono)',
                  fontSize: 'var(--text-2xs)', transform: 'translateX(-50%)',
                }}
              >
                w{w}
              </div>
          ...
          <div style={{ position: 'relative' }}>
            {ticks.map(w => (
              <div key={w} style={{ position: 'absolute', top: 0, bottom: 0, left: `${toPercent(w)}%`, width: 1, background: 'var(--border)', zIndex: 0 }} />
            ))}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {sorted.map(m => {
                const color = phaseColorMap[m.phase]
                const left = toPercent(m.gantt_start_week)
                const widthPct = toPercent(m.gantt_end_week + 1) - left

                return (
                  <div key={m.id} style={{ position: 'relative', height: 34 }}>
                    <button
                      onClick={() => router.push(`/modules/${m.id}`)}
                      style={{
                        position: 'absolute', left: `${left}%`, width: `${widthPct}%`, minWidth: 32,
                        top: 2, height: 30,
                        background: color + '25', border: `1px solid ${color}60`,
                        borderLeft: m.critical_path ? `3px solid ${color}` : `1px solid ${color}60`,
                        borderRadius: 4, color, padding: '0 8px', cursor: 'pointer', zIndex: 1,
                    ...
                    >
                      <span style={{ fontFamily: 'var(--font-mono)', opacity: 0.7, flexShrink: 0 }}>{m.id}</span>
                      {widthPct > 8 && <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{m.short_name}</span>}
                    </button>
                  </div>
                )
              })}

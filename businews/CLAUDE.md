...
### 7. Questions (two sets, tabbed)

The questions section renders as two tabs:

**Tab 1: Reflection** (default) — 5 open-ended discussion questions, unchanged from current format.

**Tab 2: IB Questions** — 5-7 structured IB BM exam-style questions with mark allocations and marking notes. Generated by the same Claude call as the case study. See `/ib-questions-guide.md` at the repo root for the full mark-level reference.

If `ib_questions` is absent from `case_study.json`, the section renders without tabs (backwards-compatible with existing cases).

### 8. Reflection Questions (exactly 5)
